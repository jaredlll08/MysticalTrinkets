...
public class TileEntityMysticalForge extends TileEntity {

	public int timer;
	public int activeTimer;

	public TileEntityMysticalForge() {
		timer = 1;
		activeTimer = 100;
	}
				...
				if (checkForBlock(xCoord + 2, yCoord, zCoord + 2, MysticalBlocks.mysticalBrick)) {
					if (checkForBlock(xCoord + 2, yCoord, zCoord - 2, MysticalBlocks.mysticalBrick)) {
						if (checkForBlock(xCoord - 2, yCoord, zCoord + 2, MysticalBlocks.mysticalBrick)) {
							...
							if (checkForBlock(xCoord - 2, yCoord + 2, zCoord - 2, MysticalBlocks.burntRitualBlock)) {
								if (checkForBlock(xCoord + 2, yCoord + 2, zCoord + 2, MysticalBlocks.burntRitualBlock)) {
									if (checkForBlock(xCoord + 2, yCoord + 2, zCoord - 2, MysticalBlocks.burntRitualBlock)) {
										if (checkForBlock(xCoord - 2, yCoord + 2, zCoord + 2, MysticalBlocks.burntRitualBlock)) {
											...
											if (activeTimer == 0) {
												...
												// yCoord, zCoord,
												// MysticalBlocks.mysticalForgeActive);
