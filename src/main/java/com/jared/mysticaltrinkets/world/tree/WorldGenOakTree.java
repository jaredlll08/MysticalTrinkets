package com.jared.mysticaltrinkets.world.tree;
...
public class WorldGenOakTree extends WorldGenAbstractTree {
	...
	public WorldGenOakTree(boolean par1) {
		this(par1, 4, 0, 0, false);
	}

	public WorldGenOakTree(boolean par1, int par2, int par3, int par4, boolean par5) {
		super(par1);
	...
	public boolean generate(World par1World, Random par2Random, int par3, int par4, int par5) {
		...
		if (par4 >= 1 && par4 + l + 1 <= 256) {
			...
			if (!flag) {
				...
				// boolean isSoil = block2.canSustainPlant(par1World, par3, par4
				// - 1, par5, ForgeDirection.UP,
				// (BlockInfusedOakTreeSapling)MysticalBlocks.infusedOakTreeSapling);
				boolean isSoil;
				if (par1World.getBlock(par3, par4 - 1, par5) == Blocks.grass || par1World.getBlock(par3, par4 - 1, par5) == Blocks.dirt || par1World.getBlock(par3, par4-1, par5)==Blocks.mycelium) {
					isSoil = true;
				} else {
					isSoil = false;
				}
				...
				if (isSoil && par4 < 256 - l - 1) {
							...
							for (int k2 = par5 - l1; k2 <= par5 + l1; ++k2) {
								...
								if (Math.abs(j2) != l1 || Math.abs(l2) != l1 || par2Random.nextInt(2) != 0 && i3 != 0) {
									...
									if (block1.isAir(par1World, i2, k1, k2) || block1.isLeaves(par1World, i2, k1, k2)) {
										this.setBlockAndNotifyAdequately(par1World, i2, k1, k2, MysticalBlocks.infusedOakLeaves, this.metaLeaves);
									}
								}
							}
						}
					...
					for (k1 = 0; k1 < l; ++k1) {
						...
						if (block.isAir(par1World, par3, par4 + k1, par5) || block.isLeaves(par1World, par3, par4 + k1, par5)) {
							this.setBlockAndNotifyAdequately(par1World, par3, par4 + k1, par5, MysticalBlocks.infusedOakLog, this.metaWood);
